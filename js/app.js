(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new core(".swiper", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".cards__left-arrow",
                nextEl: ".cards__right-arrow"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.1,
    ...
    document.querySelectorAll(".cards__icon-info");
    document.querySelectorAll(".cards__icon-info-open");
    ...
    document.addEventListener("mouseout", (event => {
        if (event.relatedTarget.closest(".menu__catalog-list")) console.log("не убираем"); else {
            itemOfUl.classList.remove("_active");
            hiddenSubMenu.classList.remove("_active");
        }
    }));
    parentIconInfo.addEventListener("mouseover", (event => {
        if (event.target.classList.contains("cards__icon-info")) cardCollection.forEach((element => {
            if (element == event.target.parentNode) {
                event.target.childNodes[1].classList.add("_active");
                if (event.target.closest(".swiper-slide-next")) event.target.childNodes[1].classList.add("_transfrm");
            }
        }));
    }));
    parentIconInfo.addEventListener("mouseout", (event => {
        if (event.target.classList.contains("cards__icon-info")) cardCollection.forEach((element => {
            if (element == event.target.parentNode) event.target.childNodes[1].classList.remove("_active");
        }));
    }));
